package eu.jstack.ablynxloader.services;

import eu.jstack.ablynxloader.exception.InvalidExcelFileException;
import eu.jstack.ablynxloader.exception.InvalidObjectFieldNameException;
...
public class ExcelToObjectMapper {
     ...
     */
    public <T> ArrayList<T> map(Class<T> cls, int sheetId) throws Exception {
        ...
        rowIterator.forEachRemaining(row -> {
            try {
                ...
                for (Field field : fields) {
                    ...
                    int index = getHeaderIndex(fieldName, workbook, sheet);
     ...
     */
    private int getHeaderIndex(String headerName, Workbook workbook, Sheet sheet) throws Exception {
        int totalColumns = sheet.rowIterator().next().getLastCellNum();
        int index = -1;
        for (index = 0; index < totalColumns; index++) {
            Cell cell = sheet.rowIterator().next().getCell(index);
